...
class Landmark(BaseSchema):
    ...
    center_offset: float = Field(default=0, description="Additonal offset to the corridor width used for layering landmark rendering. Higher values are further from the corridor midline.")
    texture: str
...
class Trial(BaseSchema):
    ...
    background_landmark_left: Landmark
    ...
    boundary_threshold: float = Field(default=-36, description="Buffer applied to far boundary of corridor to determine stopping distance.")
    end_trial_threshold: float = Field(default=-36, description="Buffer applied to far boundary of corridor to determine trial end distance.")
    maximum_trial_time: float = Field(default=180, description="Maximum amount of time to spend on this trial.") 
